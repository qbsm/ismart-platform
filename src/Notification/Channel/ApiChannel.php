...
/**
 * Отправка заявки в единый приёмник (api.ismart.pro).
 *
...
final class ApiChannel implements ChannelInterface
{
    ...
    public function send(array $formData, array $uploadedFiles, string $requestId): ChannelResult
    {
        ...
        try {
            ...
            $this->logger->error('API заявок: запрос не прошёл', [
        ...
        if ($httpCode >= 200 && $httpCode < 300 && ($decoded['ok'] ?? false)) {
            ...
            $this->logger->info('API заявок: принято', [
                'request_id' => $requestId,
                'lead' => $leadId,
        ...
        if ($httpCode >= 400 && $httpCode < 500) {
            $this->logger->warning('API заявок: отклонено', $context);
            return ChannelResult::warning($this->name(), $message, ['http_code' => $httpCode]);
        }

        $this->logger->error('API заявок: не доставлено', $context);

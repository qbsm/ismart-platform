...
## Обновление 2026-08-10: у почты появился `MAIL_ENABLE`

Расхождение с собственным правилом («у каждого канала есть `<КАНАЛ>_ENABLE`») стоило ошибки:
попытка выключить почту подменой адреса на служебный оставила канал работающим на семи
каталогах. Теперь у `MailChannel` есть флаг.

Правило разрешения намеренно трёхзначное: **флаг задан — решает флаг, флаг не задан — решает
наличие `MAIL_TO`**. Пустая строка означает «переменной в `.env` нет», а не «выключено»;
иначе выкладка нового ядра тихо оставила бы без писем каждый deployment, где флаг ещё не
прописан, — а это заявки живых клиентов. Включённый флаг всё равно требует адреса: `enable`
без `MAIL_TO` слать некуда.

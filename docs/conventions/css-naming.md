...
**Якоря шкалы — `rem` для font-size.** Правило `em` не распространяется на элементы, которые саму шкалу и задают либо жёстко связаны с коробкой в `rem`:

- `html` (`font-size: 10px` — якорь `1rem = 10px`) и `body` с полями ввода (`input`, `select`, `textarea`) в `base/typography.css` — это база, от которой считается весь `em` ниже по дереву;
- `.button` и размерные модификаторы (`base/buttons.css`) — font-size задан рядом с `height`/`padding` в `rem`; `em` расцепил бы текст с коробкой;
- компонент со своей `rem`-шкалой токенов (`--fc-font-size*` в `form-callback`) — внутри компонента шкала единая, смешивать с `em` нельзя.

Список якорей продублирован в `tools/build/check-css-units.js` (`TEXT_REM_ANCHORS`) — при добавлении нового якоря править оба места.
